...
impl Game {
    ...
    fn neighbours(&self, point: &Point) -> u8  {
        point.around().iter().filter(|p| self.board.contains(p)).count() as u8
    }

    fn step(&mut self) {
        let mut edits = Vec::<(Point, bool)>::new();
		for p in self.board.union(&self.board.iter().flat_map(|p| p.around()).collect::<HashSet<Point>>()) {
    ...
    fn tick(&mut self, ctx: &mut BTerm) {
        ...
        match ctx.key {
            ...
            Some(VirtualKeyCode::M) => {
                self.board.insert(Point::new(self.cursor_point.0 + self.reference_point.0, self.cursor_point.1 + self.reference_point.1));
            },
            // Shift viewport
            ...
            for x in 0..81 {
                match self.debug {
                    Debug::Normal => {
                        ...
                        let boardp = Point::new(drawp.x + self.reference_point.0, drawp.y + self.reference_point.1);
                        if let Some(_) = self.board.get(&boardp) {
                            draw_batch.print_color(drawp, "#", ColorPair::new(RGBA::from_u8(12, 201, 6, 255), RGBA::new()));
                        }
                    },
                    Debug::Debug(level) => {
                        let drawp = Point::new(x, y);
        ...
        draw_batch.print_color(Point::new(self.cursor_point.0, self.cursor_point.1), "@", ColorPair::new(RGBA::from_u8(12, 201, 6, 255), RGBA::new()));
